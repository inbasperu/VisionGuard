...
LandmarksEstimator::LandmarksEstimator(ov::Core &ie,
  ...
  if (outputTensorDims.size() != 4 && outputTensorDims.size() != 2) {
    throw std::runtime_error(
        modelPath + " network output layer should have 2 or 4 dimensions");
  }
}
...
cv::Mat LandmarksEstimator::affineTransform(cv::Point2f center,
                                            cv::Point2f scale, float rot,
                                            size_t dst_w, size_t dst_h,
                                            cv::Point2f shift, bool inv) {
  // Create affine transform matrix
  cv::Point2f scale_tmp = scale;
  const float pi = acos(-1.0f);
  float rot_rad = pi * rot / 180;
  cv::Point2f src_dir =
      rotatePoint(cv::Point2f(0.f, scale_tmp.y * -0.5f), rot_rad);
  cv::Point2f src[3];
  src[0] = cv::Point2f(center.x + scale_tmp.x * shift.x,
                       center.y + scale_tmp.y * shift.y);
  src[1] = cv::Point2f(center.x + src_dir.x + scale_tmp.x * shift.x,
                       center.y + src_dir.y + scale_tmp.y * shift.y);
...
LandmarksEstimator::getMaxPreds(std::vector<cv::Mat> heatMaps) {
  // Get maximum predictions from heat maps
  ...
  for (size_t landmarkId = 0; landmarkId < numberLandmarks; landmarkId++) {
    ...
    std::partial_sort(std::begin(indices),
                      std::begin(indices) + numberLandmarks, std::end(indices),
    ...
    if (maxVal > 0) {
      preds.push_back(cv::Point2f(static_cast<float>(idx % heatMaps[0].cols),
                                  static_cast<float>(idx / heatMaps[0].cols)));
    } else {
...
cv::Point2f LandmarksEstimator::get3rdPoint(cv::Point2f a, cv::Point2f b) {
  // Get the third point for affine transformation
  cv::Point2f direction = a - b;
  return cv::Point2f(b.x - direction.y, b.y + direction.x);
}

std::vector<cv::Point2i>
LandmarksEstimator::heatMapPostprocess(cv::Rect faceBoundingBox,
                                       cv::Mat faceCrop) {
  // Process heat maps to get landmarks
  ...
  float w = static_cast<float>(faceBoundingBox.width),
        h = static_cast<float>(faceBoundingBox.height);
  cv::Point2f center(faceBoundingBox.tl().x + w * 0.5f,
                     faceBoundingBox.tl().y + h * 0.5f);
  ...
  for (size_t landmarkId = 0; landmarkId < numberLandmarks; landmarkId++) {
    ...
    if (1 < px && px < heatMap.cols - 1 && 1 < py && py < heatMap.rows - 1) {
      float diffFirst =
          heatMap.at<float>(py, px + 1) - heatMap.at<float>(py, px - 1);
      float diffSecond =
          heatMap.at<float>(py + 1, px) - heatMap.at<float>(py - 1, px);
  ...
  cv::Mat trans = affineTransform(center, scale, 0, heatMapsDims[2],
                                  heatMapsDims[3], cv::Point2f(0., 0.), true);
...
std::vector<cv::Point2i>
LandmarksEstimator::simplePostprocess(cv::Rect faceBoundingBox,
                                      cv::Mat faceCrop) {
  // Process raw landmarks to get coordinates
  std::vector<float> rawLandmarks;
  ieWrapper.getOutputTensor(outputTensorName, rawLandmarks);
  std::vector<cv::Point2i> normedLandmarks;
  for (unsigned long i = 0; i < rawLandmarks.size() / 2; ++i) {
    int x = static_cast<int>(rawLandmarks[2 * i] * faceCrop.cols +
                             faceBoundingBox.tl().x);
    int y = static_cast<int>(rawLandmarks[2 * i + 1] * faceCrop.rows +
                             faceBoundingBox.tl().y);
    normedLandmarks.push_back(cv::Point2i(x, y));
  }
  return normedLandmarks;
}

std::vector<cv::Mat> LandmarksEstimator::split(std::vector<float> &data,
                                               const ov::Shape &shape) {
  // Split data into individual heat maps
