...
IEWrapper::IEWrapper(ov::Core &core, const std::string &modelPath,
                     const std::string &modelType,
                     const std::string &deviceName)
    : modelPath(modelPath), modelType(modelType), deviceName(deviceName),
      core(core) {
...
void IEWrapper::setExecPart() {
  ...
  for (size_t i = 0; i < inputs.size(); i++) {
    ...
    if (layerDims.size() == 4) {
      ...
      throw std::runtime_error("Unknown type of input layer layout. Expected "
                               "either 4 or 2 dimensional inputs");
    }
  }
...
void IEWrapper::setInputTensor(const std::string &tensorName,
                               const cv::Mat &image) {
  ...
  auto scaledSize = cv::Size(static_cast<int>(tensorDims[3]),
                             static_cast<int>(tensorDims[2]));
  cv::Mat resizedImage;
...
void IEWrapper::setInputTensor(const std::string &tensorName,
  ...
  auto tensorDims = input_tensors_dims_info[tensorName];
  size_t dimsProduct = std::accumulate(tensorDims.begin(), tensorDims.end(), 1,
...
void IEWrapper::getOutputTensor(const std::string &tensorName,
                                std::vector<float> &output) {
  output.clear();
  auto tensorDims = output_tensors_dims_info[tensorName];
  size_t dataSize = std::accumulate(tensorDims.begin(), tensorDims.end(), 1,
                                    std::multiplies<size_t>());
  float *buffer = infer_request.get_tensor(tensorName).data<float>();
...
const std::map<std::string, ov::Shape> &
IEWrapper::getInputTensorDimsInfo() const {
  return input_tensors_dims_info;
}
const std::map<std::string, ov::Shape> &
IEWrapper::getOutputTensorDimsInfo() const {
...
void IEWrapper::expectImageInput(const std::string &tensorName) const {
  ...
  if (dims.size() != 4 || dims[0] != 1 || dims[1] != 3) {
    throw std::runtime_error(modelPath + ": expected \"" + tensorName +
                             "\" to have dimensions 1x3xHxW");
  }
}

void IEWrapper::infer() { infer_request.infer(); }

void IEWrapper::reshape(
    const std::map<std::string, ov::Shape> &newTensorsDimsInfo) {
  if (input_tensors_dims_info.size() != newTensorsDimsInfo.size()) {
    throw std::runtime_error(
        "Mismatch in the number of tensors being reshaped");
  }
