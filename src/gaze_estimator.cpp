...
GazeEstimator::GazeEstimator(ov::Core &ie, const std::string &modelPath,
                             const std::string &deviceName, bool doRollAlign)
...
void GazeEstimator::estimate(const cv::Mat &image,
  ...
  if (rollAlign) {
    ...
    float sn =
        static_cast<float>(std::sin(static_cast<double>(roll) * CV_PI / 180.0));
