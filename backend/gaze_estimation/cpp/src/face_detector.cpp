...
FaceDetector::FaceDetector(ov::Core &core, const std::string &modelPath,
  ...
  if (outputTensorDims.size() != 4 || outputTensorDims[0] != 1 ||
      outputTensorDims[1] != 1 || outputTensorDims[3] != 7) {
    throw std::runtime_error(modelPath + ": expected \"" + outputTensorName +
                             "\" to have shape 1x1xNx7");
  }
...
std::vector<FaceInferenceResults> FaceDetector::detect(const cv::Mat &image) {
  ...
  if (enableReshape) {
    ...
    double networkAspectRatio =
        std::round(100. * inputTensorDims[3] / inputTensorDims[2]) / 100.;
    double aspectRatioThreshold = 0.01;

    if (std::fabs(imageAspectRatio - networkAspectRatio) >
        aspectRatioThreshold) {
      slog::debug << "Face Detection network is reshaped" << slog::endl;
      // Fix height and change width to make networkAspectRatio equal to
      // imageAspectRatio
      inputTensorDims[3] =
          static_cast<unsigned long>(inputTensorDims[2] * imageAspectRatio);
  ...
  for (unsigned long detectionID = 0; detectionID < numTotalDetections;
       ++detectionID) {
    ...
    auto height =
        rawDetectionResults[detectionID * 7 + 6] * imageSize.height - y;
